...
## 6. 인터랙티브 UI 프로토타입 & 독립 디자인 가이드 작성 표준

1. **화면 ID별 독립 파일 분리 (`PAGE-[도메인]-[NN].html`)**:
   - `doc/prototype/` 디렉토리에 각 화면 ID별 독립 HTML 파일 분리 (`PAGE-PLAY-01.html`, `PAGE-TRACK-01.html`, `PAGE-LYRICS-01.html`, `PAGE-EQ-01.html`).
2. **독립 Living Design System (`05-디자인가이드.html`)**:
   - `doc/` 루트에 위치하며 5대 멀티 테마 실시간 전환 스위처, CSS 변수 토큰 복사 및 UI 컴포넌트 쇼케이스 제공.
...
| `v2.1` | 2026-09-05 | 화면 ID별 독립 프로토타입(`prototype/`) 및 독립 디자인 가이드(`05-디자인가이드.html`) 표준화 | 모바일 UI/UX팀 |
| `v2.3` | 2026-09-06 | 오픈소스 라이선스 검증 표준 및 00~09 산출물 연동 최신화 | 모바일 UI/UX팀 |
